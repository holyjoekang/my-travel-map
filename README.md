...
## 앨범 링크 (사진은 구글 포토·드라이브에 두고 주소만 건다)

사진 파일은 저장소에 넣지 않는다. 폰이나 구글 드라이브에 둔 앨범의 **공유 주소**를 여정에 붙이면,
그 여정 패널에 `📷 앨범` 단추가 뜬다. 공개/개인은 **주소를 어느 파일에 적느냐**로 갈린다.

| | 어디에 적나 | 어디에 뜨나 |
|---|---|---|
| 공개 | `data/trips.json` 의 그 여정에 `"album"` | 공개본 · 개인 빌드 모두 |
| 개인 | `data/private.json`(gitignore) 의 `"albums": { "여정id": … }` | **개인 빌드에만** (`나만 보기` 표시) |

```jsonc
// 공개 — data/trips.json
{ "id": "t-2026-06-03", "album": "https://photos.app.goo.gl/xxxx", ... }

// 개인 — data/private.json  (주소 하나, {url,label}, 또는 그 목록)
"albums": {
  "t-2017-07-11": [
    { "url": "https://drive.google.com/drive/folders/…", "label": "폰 사진" },
    "https://photos.app.goo.gl/yyyy"
  ]
}
```

- 구글 포토: 앨범 → 공유 → 링크 만들기. 드라이브: 폴더 → 공유 → 링크가 있는 모든 사용자(뷰어)
- **개인 앨범 주소를 `trips.json` 에 적지 않는다** — 그 파일은 공개 저장소에 올라간다.
  `private.json` 에 적으면 공개 빌드는 그 파일을 번들에 넣지 않고, 새어 나가면 빌드가 실패한다
- `http(s)://` 가 아닌 주소는 빌드가 거부한다
- 공개 앨범도 링크를 아는 사람은 누구나 볼 수 있다. 정말 남에게 보이기 싫으면 개인 쪽에 적는다
