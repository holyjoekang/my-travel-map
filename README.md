...
- **홈** — 다녀온 도시가 점으로 켜지는 **지구본 표지**와 숫자 띠, **다시 가고 싶은 곳**(3D 투어 · 격자로 보면 대표 도시가 큰 카드로),
  최근의 여정, 안내할 수 있는 코스, 동행 문의. 문구와 대표 도시와 코스는 `data/site.json` 에서 고친다
  (아래 [지구본 표지](#지구본-표지-곁들이), [홈 표지](#홈-표지-원화-한-장에서), [3D 여행지 투어](#3d-여행지-투어-곁들이))
...
(홈 표지의 지구본도 같은 로더를 쓴다.)

## 지구본 표지 (곁들이)

홈의 첫 화면은 다녀온 도시 99곳을 점으로 찍은 지구본이다. 점 크기는 다녀온 횟수,
처음 들어오면 1996년부터 처음 간 해의 순서대로 점이 켜지고 숫자 띠가 같이 오른다.
연도 막대를 끌면 그해의 지구본이 되고, 점을 누르면 그 도시의 기록이 열린다.
다음 예정 여정은 서울에서 이은 점선 호로 그어진다.

- 먼저 번들의 국경선으로 그린 **정지 지구본**이 서고, three.js 를 받으면 돌아가는 지구본이 그 위에 겹친다.
  못 받아도 표지는 비지 않는다 (PRD §19)
- three.js 는 3D 투어와 **같은 로더로 한 번만** 받는다. 지구본 라이브러리·텍스처 사진은 받지 않는다
- 색은 `--globe-*` 토큰 — 밝은/어두운 테마를 따른다. 처음 앞을 보는 곳은 `G_LAT0`·`G_LNG0`
...
- 사진이 없는 도시 6곳 — 남해·부여·유명산·평택과 뉴저지·뉴헤이븐.
  trip.com 에 가이드가 없어 여행지 목록에는 이름만 놓인 자리로 나온다.
  가이드가 생기면 `data/tripcom_places.json` 에 번호를 넣고
  `fetch_destinations.py` 를 돌리면 사진이 붙는다
